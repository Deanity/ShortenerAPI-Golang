...
func (c *LinkCache) SetLink(ctx context.Context, shortCode, originalURL string) error {
	return c.rdb.Set(ctx, redirectKey(shortCode), originalURL, redirectTTL).Err()
}

// GetLink retrieves the cached originalURL for shortCode.
// Returns ("", nil) if the key does not exist (cache miss).
func (c *LinkCache) GetLink(ctx context.Context, shortCode string) (string, error) {
	val, err := c.rdb.Get(ctx, redirectKey(shortCode)).Result()
	if err == redis.Nil {
...
func (c *LinkCache) DeleteLink(ctx context.Context, shortCode string) error {
	return c.rdb.Del(ctx, redirectKey(shortCode)).Err()
}
...
func (c *LinkCache) IsUniqueVisitor(ctx context.Context, shortCode, ipHash string) (bool, error) {
	key := fmt.Sprintf("uniq:%s:%s", shortCode, ipHash)
...
func redirectKey(shortCode string) string {
